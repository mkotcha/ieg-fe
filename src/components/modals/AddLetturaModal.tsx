...
const AddLetturaModal = () => {
  const showAddModal = useAppSelector(state => state.modal.showAddLetturaModal);
  const modLetturaId = useAppSelector(state => state.modal.modLetturaId);
  const token = useAppSelector(state => state.auth.token);
  const [listaPod, setListaPod] = useState<string[] | null>(null);
  const [lettura, setLettura] = useState<Record<string, string | number>>({
    dataLettura: "",
  ...
  useEffect(() => {
    ...
    const fetchLettura = async () => {
      ...
      const response = await axios.get<Lettura>(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setLettura(response.data as unknown as Record<string, string | number>);
    };
