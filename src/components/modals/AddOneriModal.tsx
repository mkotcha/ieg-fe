...
const AddOneriModal = () => {
  ...
  const modOneriId = useAppSelector(state => state.modal.modOneriId);
  const token = useAppSelector(state => state.auth.token);

  const [oneri, setOneri] = useState<Record<string, string | number>>({
    tipo: "BTA1",
    qeTud: 0,
    qpTdm: 0,
    qfTud: 0,
  ...
  useEffect(() => {
    const fetchOneri = async () => {
      ...
      const response = await axios.get<Oneri>(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setOneri(response.data as unknown as Record<string, string | number>);
    };
